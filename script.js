...
(function(){
  ...
  function render(){
    ...
    for(let i=0;i<firstDay;i++){ const pad=document.createElement('div'); pad.className='day'; pad.innerHTML='<div class="date"> </div>'; grid.appendChild(pad); }
  ...
  // Day editor state/refs
  const dayEditor = (()=> {
    const dlg = document.getElementById('dayEditor');
    ...
    function openFor(date){
      ...
      list.innerHTML = '';
      entries(editingDate).forEach(e=> list.appendChild(rowTemplate(e.label, e.amount)));
      if (!entries(editingDate).length) list.appendChild(rowTemplate('', ''));
      dlg.showModal();
    }
  ...
  const url = URL.createObjectURL(blob);
    ...
    a.href = url; a.download = "goal-tracking-calendar-data.json"; a.click();
  ...
  inp.type = "file"; inp.accept = "application/json";
    inp.onchange = async()=>{
      ...
      try{
        ...
        if(obj.data) store = obj.data;
        saveJSON(SETTINGS_KEY, settings); saveJSON(DATA_KEY, store);
